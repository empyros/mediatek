...
//Burst Write Data
static int iWriteData(unsigned int  ui4_offset, unsigned int  ui4_length, unsigned short * pinputdata)
{
   int  i4RetValue = 0;
...
static int Enb_OTP_Read(int enb){
	...
	u8 * reVal;
        char puSendCmd[3] = {(char)(0x0100 >> 8) , (char)(0x0100 & 0xFF) ,(char)(0x01)};
...
CAM_CALDB("[CAM_CAL]Read=%x,%x\n",reVal[0],reVal[1]);
	...
	if (cnt == 50)
		{
		 CAM_CALDB("[S24CAM_CAL] start read fail.\n");
			return 0;
		}
		CAM_CALDB("[S24CAM_CAL] start read ok.\n");
		return 1;
	} 
	else 
	{
                 i2c_master_send(g_pstI2Cclient, puSendCmd, 3);
                // *pOneByteBuff = 0x0614;
		// iWriteData(0x301A, 1, pOneByteBuff );
                 mdelay(5); 
	     kfree(pOneByteBuff);
