...
*
*************************************************************************/
kal_int8 OV5647_ReadID(void)
{
	kal_uint16 sensor_id=0; 
	kal_int8  retry = 2;
	UINT8 ret = 0;

	/*PDN pin Low: Ov5647, High OV5647_AC*/
	
	do
	{
		sensor_id=((OV5647_read_cmos_sensor(0x300A) << 8) | OV5647_read_cmos_sensor(0x300B));	

		printk("OV5647_G04a ReadId	, sensor_id:%x \n",sensor_id);
...
UINT32 OV5647GetResolution(MSDK_SENSOR_RESOLUTION_INFO_STRUCT *pSensorResolution)
{
	...
	pSensorResolution->SensorVideoWidth = OV5647_IMAGE_SENSOR_PV_WIDTH;
	pSensorResolution->SensorVideoHeight = OV5647_IMAGE_SENSOR_PV_HEIGHT;		
	return ERROR_NONE;
}	/* OV5647GetResolution() */
