...
extern bool slid_flag;
extern int mhall_time;
extern  int  mhall_time_lcm;
static int tpd_load_status_sub=0,tpd_load_status_main=0;
extern void lcm_backlight_switch(void);
//////////////////////////////////////////////////////////////////////////////////
...
{
#if defined(I5000_N618) && defined(N618_XINJIDA_CTP_MODULE)
#include "ft_app_xinjida.i"
#elif defined(I5000_A10)  //amy0613
#include "ft_app_chenxing.i"
#elif defined(I5000_P150) && defined(P150_XINLI_CT0983_MODULE)
#include "ft_app_xinli.i"
#elif defined(I6000_I6050)
#include "ft_app_yijian.i"
#else
#include "ft_app.i"
#endif
};
...
#if defined(I5000_N618)||defined(I5000_P150)||defined(I6000_I6050)
...
#if ((!defined(PHILIPS_ATLAS))&&(!defined(SIMCOM_I5000)))
	 ...
	 do
	 {
		...
		//printk("[LONG]ESD ERROR:TP ESD DATA=%x\r\n",esd_data);
		  ...
		  TPD_DEBUG("point_num = %d\n",point_num);
			...
			{         
				//if((x_history[cinfo.id[idx]]==cinfo.x[idx])&&(y_history[cinfo.id[idx]]==cinfo.y[idx])&&(cinfo.p[idx]==2))
				//{
					//printk("[LONG]long press");
				//}
				//else
				{
					...
					{
						#ifdef TPD_HAVE_BUTTON
						if((cinfo.p[idx]==2)&&(x_history[cinfo.id[idx]]<TPD_X_RES)&&(y_history[cinfo.id[idx]]<TPD_Y_RES))
						{
							tpd_up(TPD_WARP_X(x_history[idx]), TPD_WARP_Y(y_history[idx]), 0,idx); 
						}
						tpd_button(cinfo.x[idx], cinfo.y[idx], 1);
						#endif
					}

				}

				cur_mask |= ( 1 << cinfo.id[idx] );
				x_history[cinfo.id[idx]]=cinfo.x[idx];
				y_history[cinfo.id[idx]]=cinfo.y[idx];         
				event_history[cinfo.id[idx]] =cinfo.p[idx];
			}         
			...
			if ( cur_mask != id_mask )
			{
				     ...
				     ( ( cur_mask >> idx ) & 0x01 ) == 0 )
				{
					...
					if (x_history[idx] < TPD_X_RES && y_history[idx] < TPD_Y_RES)
					...
					{
						#ifdef TPD_HAVE_BUTTON
						tpd_button(cinfo.x[x_history[idx]], cinfo.y[y_history[idx]], 0);
						#endif
					}            
...
//for two tp add code  maliyan 
static bool  tp_sub_suspend_flag=false;
static bool  tp_main_suspend_flag=false;
 static int tpd_resume_sub(struct i2c_client *client)
 {
  int retval = TPD_OK;

       TPD_DEBUG("[mly]]TPD sub wake up\n");

      printk("[mtpd]tpd_resume_sub\n");
	...
	mt_set_gpio_mode(GPIO_CTP_RST2_PIN, GPIO_CTP_RST_PIN_M_GPIO);
	mt_set_gpio_dir(GPIO_CTP_RST2_PIN, GPIO_DIR_OUT);
	mt_set_gpio_out(GPIO_CTP_RST2_PIN, GPIO_OUT_ZERO);  
	msleep(10);  
	mt_set_gpio_mode(GPIO_CTP_RST2_PIN, GPIO_CTP_RST_PIN_M_GPIO);
	mt_set_gpio_dir(GPIO_CTP_RST2_PIN, GPIO_DIR_OUT);
	mt_set_gpio_out(GPIO_CTP_RST2_PIN, GPIO_OUT_ONE);

	msleep(300);//350-->300
#endif
	mt65xx_eint_unmask(CUST_EINT_TOUCH_PANEL2_NUM);  
#if defined(ESD_CHECK)
	hrtimer_start(&ft5306_timer, ktime_set(0, 500000000), HRTIMER_MODE_REL); //check again after 500ms
#endif	
      i2c_client->addr=0x72>>1;
     tp_sub_suspend_flag=false;
     //	tpd_halt = 0;
	 return retval;
 }
 
 static int tpd_suspend_sub(struct i2c_client *client)
 {
	 int retval = TPD_OK;
	 static char data = 0x3;
 
	 TPD_DEBUG("[mly]TPD sub enter sleep\n");
	 
	 printk("[mtpd]tpd_suspend_sub\n");
...
#endif
	 mt65xx_eint_mask(CUST_EINT_TOUCH_PANEL2_NUM);
#ifdef TPD_CLOSE_POWER_IN_SLEEP	
	hwPowerDown(TPD_POWER_SOURCE,"TP");
#else
...
#endif
      tp_sub_suspend_flag=true;
	 return retval;
 } 

 static int tpd_resume_main(struct i2c_client *client)
 {
  int retval = TPD_OK;
 
   TPD_DEBUG("[mly]TPD main wake up\n");
   printk("[mtpd]tpd_resume_main\n");
	...
	hrtimer_start(&ft5306_timer, ktime_set(0, 500000000), HRTIMER_MODE_REL); //check again after 500ms
     ...
     tp_main_suspend_flag=false;
     	//tpd_halt = 0;    	
	 return retval;
 }
 
 static int tpd_suspend_main(struct i2c_client *client)
 {
	 int retval = TPD_OK;
	 static char data = 0x3;
 
	 TPD_DEBUG("[mly]TPD main enter sleep\n");
	  printk("[mtpd]tpd_suspend_main\n");
...
void tp_switch(void)
{
	...
	if (slid_flag)//close
	{
		if(!tp_main_suspend_flag)
		{
		tpd_suspend_main(i2c_client);
		msleep(10);
		}
tpd_resume_sub(i2c_client);
}
else //open
{
if(!tp_sub_suspend_flag)
		{
		tpd_suspend_sub(i2c_client);
		msleep(10);
		}
tpd_resume_main(i2c_client);
}
}

 static int tpd_init_sub(struct i2c_client *client)
 {	 
	int retval = TPD_OK;
	char data;
	u8 report_rate=0;
...
reset_proc:   
	...
	mt_set_gpio_mode(GPIO_CTP_EINT2_PIN, GPIO_CTP_EINT_PIN_M_EINT);
    mt_set_gpio_dir(GPIO_CTP_EINT2_PIN, GPIO_DIR_IN);
    mt_set_gpio_pull_enable(GPIO_CTP_EINT2_PIN, GPIO_PULL_ENABLE);
    mt_set_gpio_pull_select(GPIO_CTP_EINT2_PIN, GPIO_PULL_UP);
 
	  mt65xx_eint_set_sens(CUST_EINT_TOUCH_PANEL2_NUM, CUST_EINT_TOUCH_PANEL2_SENSITIVE);
	  mt65xx_eint_set_hw_debounce(CUST_EINT_TOUCH_PANEL2_NUM, CUST_EINT_TOUCH_PANEL2_DEBOUNCE_CN);
	  mt65xx_eint_registration(CUST_EINT_TOUCH_PANEL2_NUM, CUST_EINT_TOUCH_PANEL2_DEBOUNCE_EN, CUST_EINT_TOUCH_PANEL2_POLARITY, tpd_eint_interrupt_handler, 1); 
	  mt65xx_eint_unmask(CUST_EINT_TOUCH_PANEL2_NUM);
...
#if def TPD_RESET_ISSUE_WORKAROUND
	...
	i2c_smbus_read_i2c_block_data(i2c_client, FT5X0X_REG_FT5201ID, 1, &tp_id);
...
static int tpd_init_main(struct i2c_client *client)
 {	 
	int retval = TPD_OK;
	char data;
	u8 report_rate=0;
...
reset_proc:   
 ...
 {	 
   ...
   // hwPowerOn(MT65XX_POWER_LDO_VGP2, VOL_2800, "TP");
    hwPowerOn(TPD_POWER_SOURCE, VOL_2800, "TP");      //MT65XX_POWER_LDO_VGP=2.8v
        ...
        tpd_init_sub(client);
      // tpd_suspend_sub(client);
        tpd_init_main(client);
	//tpd_suspend_main(client);
	 //tpd_halt = 1; 
      //  if((tpd_load_status_sub==1)||(tpd_load_status_main==1))
	tpd_load_status = 1;
 ...
 {
  int retval = TPD_OK;
      printk("[mtpd]tpd_resume:slid_flag %d\n",slid_flag);
     if (slid_flag)//close
      tpd_resume_sub(client);
     else //open   
     {
        if (mhall_time==0)
       {
           mhall_time_lcm=mhall_time;
            lcm_backlight_switch();
		mhall_time=2;	
       }
	tpd_resume_main(client);
 ...
 static int tpd_suspend(struct i2c_client *client, pm_message_t message)
 {
	 int retval = TPD_OK;
	  ...
	  tpd_halt = 1; 
       printk("[mtpd]tpd_suspend: tp_sub_suspend_flag=%d,tp_main_suspend_flag=%d\n",tp_sub_suspend_flag,tp_main_suspend_flag);
...
void tpd_led_set_en(int onoff)
{
	 if (onoff==1)
	 {
		mt_set_gpio_mode(GPIO_CTP_LED_EN_PIN, GPIO_CTP_LED_EN_PIN_M_GPIO);
		mt_set_gpio_dir(GPIO_CTP_LED_EN_PIN, GPIO_DIR_OUT);
		mt_set_gpio_out(GPIO_CTP_LED_EN_PIN, GPIO_OUT_ONE);  
		msleep(5); 
	 }
	else
	{
		mt_set_gpio_mode(GPIO_CTP_LED_EN_PIN, GPIO_CTP_LED_EN_PIN_M_GPIO);
		mt_set_gpio_dir(GPIO_CTP_LED_EN_PIN, GPIO_DIR_OUT);
		mt_set_gpio_out(GPIO_CTP_LED_EN_PIN, GPIO_OUT_ZERO);  
		msleep(5);  
	}
}
