...
extern mhall_flag;  //0 --open ,1--slid
extern int tp_state;
/*0---tp power=2.8v,tp is working
  1---tp power=0v,tp sleep
  2---tp has not init  */

void ft5336_suspend(void)
{
	tpd_halt = 1;
...
EXPORT_SYMBOL(ft5336_suspend);

void ft5336_resume(void)
{
	hwPowerOn(TPD_POWER_SOURCE_CUSTOM,VOL_2800,"TP");
	mt65xx_eint_unmask(CUST_EINT_TOUCH_PANEL_NUM); 
	//msleep(30);
...
static int tpd_local_init(void)
{
	...
	#if defined(PHILIPS_STAMFORD)
	TPD_DMESG("[lxw]tp init done,tp power on\n");
	tp_state=0;
	if((tp_state==0)&&(mhall_flag==1))  //mhall is close  ,tp sleep
	{
		ft5336_suspend();
		TPD_DMESG(TPD_DEVICE "[lxw]tp init---close,sleep \n"); 
	}
	#endif	
#ifdef TPD_HAVE_BUTTON     
/*
 ...
 {
#if defined(PHILIPS_STAMFORD)
	if((tp_state==1)&&(mhall_flag==0))  //mhall is open and tp sleep ,tp resume
	{
		ft5336_resume();
		TPD_DMESG(TPD_DEVICE "[lxw]tpd_resume---open resume \n"); 
	}
	else
		TPD_DMESG(TPD_DEVICE "[lxw]tpd_resume---do nothing \n"); 
  ...
  //char data;
   ...
   // mt_set_gpio_dir(GPIO_CTP_RST_PIN, GPIO_DIR_OUT);
    mt_set_gpio_out(GPIO_CTP_RST_PIN, GPIO_OUT_ONE);
#endif
	msleep(200);//add this line  //longxuewei
	mt65xx_eint_unmask(CUST_EINT_TOUCH_PANEL_NUM);  
#ifdef USB_CHARGE_DETECT
...
#if defined(PHILIPS_STAMFORD)
	if(tp_state==0)  //tp is working ,tp sleep
	{
		ft5336_suspend();
		TPD_DMESG(TPD_DEVICE "[lxw]tpd_suspend---suspend \n"); 
	}
	else
		TPD_DMESG(TPD_DEVICE "[lxw]tpd_suspend---do nothing \n"); 
