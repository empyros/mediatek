...
static int led_blink_pmic(enum mt65xx_led_pmic pmic_type, struct nled_setting* led) {
	...
	switch(pmic_type){
		...
		case MT65XX_LED_PMIC_NLED_ISINK1://keypad LR  16mA
			...
			upmu_set_isinks_ch1_step(0x0);//0x0==>4mA
			#else
			upmu_set_isinks_ch1_step(0x3);//0x3==>16mA
			#endif
		...
		case MT65XX_LED_PMIC_NLED_ISINK2://keypad LR  16mA
			...
			#if defined(PHILIPS_ROYAL2)
			upmu_set_isinks_ch2_step(0x0);//0x0==>4mA
			#else
			upmu_set_isinks_ch2_step(0x3);//0x3==>16mA
			#endif
...
extern void tpd_led_set_en(int onoff);
#endif
static int brightness_set_pmic(enum mt65xx_led_pmic pmic_type, u32 level, u32 div)
{
		//int tmp_level = level;
