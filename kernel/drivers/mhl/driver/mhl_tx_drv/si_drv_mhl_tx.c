...
uint8_t hidbi = 0;
uint8_t hidbt = 0;
uint8_t hidbuf[HID_BUF * HID_SIZE];
static struct input_dev *mhl_hid_dev = NULL;
struct tpd_device {
...
__u32 kb_map[KB_LEN] = { 0,0,0,0,
  ...
  KEY_BACKSLASH, KEY_LEFTMETA, KEY_POWER, KEY_KPEQUAL, KEY_F13, KEY_F14, KEY_F15, KEY_F16, KEY_F17, KEY_F18,
  KEY_F19, KEY_F20, KEY_F21, KEY_F22, KEY_F23, KEY_F24
};


static int mhl_hid_init(void) {
	int err, i;
	...
	//set_bit(EV_ABS, mhl_hid_dev->evbit); // for mouse
	set_bit(EV_REL, mhl_hid_dev->evbit); // for mouse
	set_bit(REL_X, mhl_hid_dev->relbit);
	set_bit(REL_Y, mhl_hid_dev->relbit);
	set_bit(REL_WHEEL, mhl_hid_dev->relbit);
    set_bit(BTN_LEFT, mhl_hid_dev->keybit);
    set_bit(BTN_MIDDLE, mhl_hid_dev->keybit);
    set_bit(BTN_RIGHT, mhl_hid_dev->keybit);
    set_bit(BTN_LEFT, mhl_hid_dev->keybit);
    set_bit(BTN_MOUSE, mhl_hid_dev->keybit);

	//set_bit(EV_ABS, mhl_hid_dev->evbit); // for touchpad, not used for now
	for(i=0;i<KB_LEN;i++) set_bit(kb_map[i], mhl_hid_dev->keybit);
	for(i=0;i<KB_MODLEN;i++) set_bit(kb_modmap[i], mhl_hid_dev->keybit);
...
static void mhl_hid_crypt(unsigned char mid, unsigned char cpc) {
  unsigned char cps[6];
  unsigned char enc;
  unsigned int i;
  for(i=0;i<6;i++) {
...
static void mhl_hid_kb(void) {
	int i, j, k, kb_update[4] = {0,0,0,0};
    uint8_t *chidbuf = &(hidbuf[hidbi]);
	int mod_update = kb_modifier ^ chidbuf[2];
    //for(i=2;i<7;i++) printk("%2x ", hidbuf[i]); printk("\n");
    if(chidbuf[3]==0x1 && chidbuf[4]==0x1 && chidbuf[5]==0x1) return;
	for(i=0;i<8;i++) {
		if(mod_update&(1<<i)) input_report_key(mhl_hid_dev, kb_modmap[i], ((chidbuf[2]&(1<<i))?1:0));
	}
    kb_modifier = chidbuf[2];
	for(i=3;i<7;i++) {
        if(chidbuf[i]==0) break;
		for(j=0;j<4;j++) if(kb_codes[j]==chidbuf[i]) break;
		if(j==4) input_report_key(mhl_hid_dev, kb_map[chidbuf[i]<116?chidbuf[i]:0], 1);
		else kb_codes[j] = 0;
	}
	for(i=0;i<4;i++) if(kb_codes[i]) input_report_key(mhl_hid_dev, kb_map[kb_codes[i]], 0);
    for(i=0;i<4;i++) kb_codes[i] = chidbuf[i+3];
    /*printk("[>>kirby<<]");
    for(i=0;i<16;i++) {
      printk("%02d:%02x ", i, chidbuf[i]);
    } printk("|||");
    for(i=0;i<4;i++) printk("%02x ", kb_codes[i]);*/
    //printk("\n");
	input_sync(mhl_hid_dev);
    //mtkfb_set_backlight_level(0);
}

static void mhl_hid_mouse(void) {
    int x,y,z,i,tmp_btn;
    uint8_t *chidbuf = &(hidbuf[hidbi]);
    ...
    mouse_btn = chidbuf[2];
    /*if(x>10 || x<-10) x=(x*222)/100;
    else x = ( x * (1500 + (77*(x>0?x:-x)) ))/1000;
    if(y>10 || y<-10) y=(y*222)/100;
    else y = ( y * (1500 + (77*(y>0?y:-y)) ))/1000;*/
    x = x * 2; y = y * 2;
	mx += x, my += y;
	if(mx<0) mx = 0;
	if(my<0) my = 0;
    if(mx>HID_RES_X) mx = HID_RES_X;
    if(my>HID_RES_Y) my = HID_RES_Y;
	//printk("m(%03d %03d) d(%03d %03d) w(%03d) btn(%2x) | ",mx, my, x, y, z, mouse_btn);

#ifndef MHL_HID_AS_TOUCH
    for(i=0;i<3;i++) if(tmp_btn&(1<<i)) input_report_key(mhl_hid_dev, mouse_btns[i], chidbuf[2]&(1<<i));
	//input_report_key(mhl_hid_dev, BTN_LEFT, (mouse_btn?1:0));
	input_report_rel(mhl_hid_dev, REL_X, x);
	input_report_rel(mhl_hid_dev, REL_Y, y);
	input_report_rel(mhl_hid_dev, REL_WHEEL, z);
	input_sync(mhl_hid_dev);
#else
    input_report_key(tpd->dev, BTN_TOUCH, (mouse_btn?1:0));
    if(mouse_btn) {
	  input_report_abs(tpd->dev, ABS_MT_POSITION_X, mx);
	  input_report_abs(tpd->dev, ABS_MT_POSITION_Y, my);
    }
...
#ifndef USE_EARLYSUSPEND
	if(isSuspend == flag) return;
	isSuspend = flag;
    doSuspend = 1;
    printk("[kirby] [isSuspend:%d] [doSuspend:%d]\n", isSuspend, doSuspend);
    wake_up_interruptible(&mhl_hid_wq);
    //disp_bls_set_backlight(isSuspend?1:255);
#else
    int i;
    uint8_t *chidbuf = &(hidbuf[hidbt]);
	if(isSuspend == flag) return;
	isSuspend = flag;
    if(unlikely(HID_RES_Y==0)) mhl_hid_init();
	input_report_key(mhl_hid_dev, KEY_POWER, 1);
	input_sync(mhl_hid_dev);
    msleep(1);
	input_report_key(mhl_hid_dev, KEY_POWER, 0);
	input_sync(mhl_hid_dev);
    msleep(30);
    if(isSuspend==1) wake_lock(&smartbook_suspend_lock);
    else wake_unlock(&smartbook_suspend_lock);
#endif
}
...
int SiiHidWrite(int key) {
    ...
    if(unlikely(HID_RES_Y==0)) mhl_hid_init();
	//K1 = get_jiffies_64();
	//SiiRegReadBlock(REG_CBUS_SCRATCHPAD_0, chidbuf, HID_SIZE);
	if(key==0) SiiRegReadBlock(REG_CBUS_SCRATCHPAD_0, chidbuf, 8);//HID_SIZE);
    else {
    ...
    if(unlikely(hidbi==hidbt)) {
        printk("[kirby] MHL Hid Ring buf Overflow. ");
...
int SiiHidRead() {
    ...
    //if(hid_mid[hid_kid]<0x10) mhl_hid_crypt(hid_mid[hid_kid], hid_cpc[hid_kid]);
	//printk("\n --- [ %d ] ---\n\n", (int)(K2-K1));
    //printk("kirby[%d][%d] ", hidbuf[0], hidbuf[1]);
    if(chidbuf[0]==1 && chidbuf[1]==2) mhl_hid_mouse();
	else if (chidbuf[0]==1 && chidbuf[1]==3) mhl_hid_kb();
    /*else if (hidbuf[0]&0x80 && hidbuf[1]==3) {
      if(hidbuf[3]==2) update_battery_2nd_info(POWER_SUPPLY_STATUS_FULL, hidbuf[2], 1);
      else if(hidbuf[3]==4) update_battery_2nd_info(POWER_SUPPLY_STATUS_CHARGING, hidbuf[2], 1);
      else if(hidbuf[3]==8) update_battery_2nd_info(POWER_SUPPLY_STATUS_NOT_CHARGING, hidbuf[2], 1); // presume 8==not charging
    }*/
    hidbi = (hidbi + HID_SIZE)%(HID_SIZE * HID_BUF);
}


int mhl_hid_kthread(void *data) {
    int i;
    wake_lock_init(&smartbook_suspend_lock, WAKE_LOCK_SUSPEND, "smartbook wakelock");
    while(1) {
        if(hidbt==hidbi) {
          set_current_state(TASK_INTERRUPTIBLE);
          wait_event_interruptible(mhl_hid_wq, hidbt!=hidbi || doSuspend);
          set_current_state(TASK_RUNNING);
        }
        if(doSuspend==1) {
          doSuspend = 0;
          printk("[kirby] wait to change backlight [isSuspend:%d]\n", isSuspend);
          if(isSuspend) ssleep(7);
          else msleep(500);
          printk("[kirby] changing backlight [isSuspend:%d]\n", isSuspend);
          disp_bls_set_backlight(isSuspend?0:255);
        } else SiiHidRead();
...
void 	SiiMhlTxDeviceIsr( void )
{
    ...
    do
    {
	    /*if(SiiRegRead(REG_CBUS_MSC_INT2_STATUS)==BIT0) {
			SiiRegWrite(REG_CBUS_MSC_INT2_STATUS, BIT0);
			SiiMhlTxMscWriteBurstDone( 1 );
			break;
		}*/
