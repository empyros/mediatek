...
static struct task_struct *mhl_hid_task = NULL; //add by kirby
int mhl_hid_kthread(void *data);

static wait_queue_head_t mhl_irq_wq;
extern wait_queue_head_t mhl_hid_wq;
...
halReturn_t HalInstallIrqHandler(fwIrqHandler_t irqHandler)
{
    ...
    //add by kirby
    init_waitqueue_head(&mhl_hid_wq);
	
	mhl_irq_task = kthread_create(mhl_irq_kthread, NULL, "hdmi_update_kthread"); 
	wake_up_process(mhl_irq_task);

    //add by kirby
	mhl_hid_task = kthread_create(mhl_hid_kthread, NULL, "hdmi_hid_kthread"); 
	wake_up_process(mhl_hid_task);
