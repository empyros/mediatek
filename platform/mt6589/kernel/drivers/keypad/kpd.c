...
static void mhall_switch(struct work_struct *work);
static DECLARE_WORK(mhall_work, mhall_switch);
#endif
/* for MR */
#if (KPD_DRV_CTRL_MHALL||KPD_DRV_CTRL_MHALL2)
...
static void kpd_mhall_handler(unsigned long data)
{
    ...
    if (mhall) // mhall closed
    {
        if (!kpd_suspend) 
        {
	     ...
	     input_sync(kpd_input_dev); //amy0615 
...
#if KPD_DRV_CTRL_MHALL2
#if defined(PHILIPS_ROYAL2)

extern void lcm_backlight_switch(void);
extern void tp_switch(void);
extern bool slid__lcm_flag;
extern  int  mhall_time_lcm;
bool slid_flag=false;
int mhall_time=0;
u8 old_state_l = 0;

static void mhall_switch(struct work_struct *work)
{
     printk("[mly]mhall_work_callback\n" );  
	...
	/* for detecting the return to old_state */
	printk("[mly]before mt65xx_eint_set_polarity!\n");  		
	mt65xx_eint_set_polarity(KPD_MHALL_EINT, old_state_l);
	mt65xx_eint_unmask(KPD_MHALL_EINT);

 }
#endif

#if defined(PHILIPS_STAMFORD)
extern void ft5336_suspend(void);
extern void ft5336_resume(void);
int mhall_flag=0;  //0 --open ,1--close
int tp_state=2;
/*0---tp power=2.8v,tp is working
  1---tp power=0v,tp sleep
  2---tp has not init*/
#endif
...
static void kpd_mhall_handler(unsigned long data)
{
	...
	//sw_mhall= !mhall;
	/* for mhall, 0: mhall opened, 1: mhall closed */
	input_report_switch(kpd_input_dev, SW_LID, mhall);
	input_sync(kpd_input_dev);
	kpd_print("[MHALL2]report QWERTY = %s\n", mhall ? "closed":"slid");
#if defined(PHILIPS_ROYAL2)
       //printk("[mly]report QWERTY = %s;mhall_time=%d\n", sw_mhall  ? "slid" : "closed",mhall_time);
       slid_flag=mhall ;
      slid__lcm_flag=mhall ;
      old_state_l =old_state;
	//printk("[mly]slid_flag= %d,slid__lcm_flag= %d\n", slid_flag,slid__lcm_flag);
	      schedule_work(&mhall_work);
	...
	#if defined(PHILIPS_STAMFORD)
	if(mhall==1) //close, tp power off
	{
		if(!kpd_suspend) //if tp is working ,power off
		{
			if(tp_state==0)
			{
				ft5336_suspend();
				kpd_print("[MHALL2][lxw]close,tp work(2.8v),tp power off!!!!\r\n"); 
			}
			else if(tp_state==1)
			{
				//kpd_print("[MHALL2][lxw]close ,tp work(0v),do nothing\r\n"); 
			}
		}
		else
		{
			//kpd_print("[MHALL2][lxw]close ,tp sleep in,do nothing\r\n"); 
		}
	}
	else //open,tp power on
	{
		if(tp_state==1) //if tp is working ,power off
		{
			ft5336_resume();
			kpd_print("[MHALL2][lxw]open,(0v),tp power on!!!!\r\n"); 
		}
		else if(tp_state==0)
		{
			//kpd_print("[MHALL2][lxw]open ,(2.8v),do nothing\r\n"); 
		}
	}
	mhall_flag=mhall;
	#endif
...
static void kpd_lowpower_handler(unsigned long data)
{
	...
	//}
	/* for detecting the return to old_state */
	printk("[LOWPOWER]report lowpower QWERTY , -3,  set int polarity state=%d  \n", old_state);
...
#if (KPD_HAS_SLIDE_QWERTY||KPD_DRV_CTRL_MHALL2)
...
/**********************disable kpd as wake up source operation********************************/
	...
	#if defined(PHILIPS_ROYAL2)||defined(PHILIPS_STAMFORD)
	kpd_print("enable kpd as wake up source operation!\n");
	upmu_set_rg_smps_autoff_dis(0x01);//maliyan 
	#else
...
#if defined(PHILIPS_ROYAL2)||defined(PHILIPS_STAMFORD)//maliyan
#else
