...
void bq24158_dump_register(void)
{
    int i=0;
#ifdef DEBUG_BQ24159
        printk("Dump reg, debug bq24159,not bq24158 !\n");        
#else
		xxx
        printk("Dump reg, debug bq24158\n");        
#endif
...
{    
    if(g_enable_high_vbat_spec == 1)
    {
        ...
        {
	#ifdef DEBUG_BQ24159
            printk("[bq24159_hw_init] g_enable_high_vbat_spec == 1, (0x06,0xf0) because 0x1020\n");
            bq24158_config_interface_reg(0x06,0x60); // set ISAFE	
	#else			
        ...
        {
            printk("[bq24158_hw_init]  g_enable_high_vbat_spec == 1, (0x06,0x77)\n");
    ...
    {
	#ifdef DEBUG_BQ24159
        printk("[bq24159_hw_init] (0x06,0xf0) \n");
        bq24158_config_interface_reg(0x06,0x60); // set ISAFE	
	#else
