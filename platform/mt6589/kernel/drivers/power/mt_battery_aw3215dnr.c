...
void pchr_turn_off_charging_aw3215dnr (void);
void pchr_turn_on_charging_aw3215dnr (void);
...
kal_bool upmu_is_chr_det(void)
{
    ...
    if(tmp32 == 0)
    {
        xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "[upmu_is_chr_det] No charger\n");
        return KAL_FALSE;
    }
    else
    {
...
#define GPIO_CHARGER_EN_PIN        GPIO_SWCHARGER_EN_PIN
#define GPIO_CHARGER_CURRENT_PIN   GPIO19
#define GPIO_CHARGER_STAT_PIN      GPIO116
...
void select_charging_curret_bcct(void)
{
    ...
    {
        if(g_bcct_value < 100)        pchr_turn_off_charging_aw3215dnr();
        else if(g_bcct_value < 500)   aw3215dnr_config_interface_liao(0x01,0x38);
        else if(g_bcct_value < 800)   aw3215dnr_config_interface_liao(0x01,0x78);
        else if(g_bcct_value == 800)  aw3215dnr_config_interface_liao(0x01,0xB8);
        else                          aw3215dnr_config_interface_liao(0x01,0x78);     
    }
    else if( (BMT_status.charger_type == STANDARD_CHARGER) ||
             (BMT_status.charger_type == CHARGING_HOST) )
    {
        aw3215dnr_config_interface_liao(0x06,0x70); //set ISAFE
        aw3215dnr_config_interface_liao(0x01,0xF8);
        aw3215dnr_config_interface_liao(0x02,0x8E);
        
        //---------------------------------------------------
        //set IOCHARGE
        if(g_bcct_value < 550)        pchr_turn_off_charging_aw3215dnr();
        else if(g_bcct_value < 650)   aw3215dnr_config_interface_liao(0x04,0x09); 
        else if(g_bcct_value < 750)   aw3215dnr_config_interface_liao(0x04,0x19);
        else if(g_bcct_value < 850)   aw3215dnr_config_interface_liao(0x04,0x29);
        else if(g_bcct_value < 950)   aw3215dnr_config_interface_liao(0x04,0x39);
        else if(g_bcct_value < 1050)  aw3215dnr_config_interface_liao(0x04,0x49);
        else if(g_bcct_value < 1150)  aw3215dnr_config_interface_liao(0x04,0x59);
        else if(g_bcct_value < 1250)  aw3215dnr_config_interface_liao(0x04,0x69);
        else if(g_bcct_value == 1250) aw3215dnr_config_interface_liao(0x04,0x79);
        else                          aw3215dnr_config_interface_liao(0x04,0x19);
        //---------------------------------------------------
        
        aw3215dnr_config_interface_liao(0x05,0x04);
    }
    else
    {
        aw3215dnr_config_interface_liao(0x01,0x78);        
    }
#endif	
}

extern kal_uint32 aw3215dnr_read_interface (kal_uint8 RegNum, kal_uint8 *val, kal_uint8 MASK, kal_uint8 SHIFT);
...
int get_bat_charging_current_level(void)
{
    ...
    {
        //Get current level
        aw3215dnr_read_interface(0x1, &ret_val, 0x3, 0x6);
    ...
    {
        //Get current level
        aw3215dnr_read_interface(0x4, &ret_val, 0x7, 0x4);
...
{
    printk("[BATTERY:aw3215dnr] set_bat_charging_current_limit (%d)\r\n", current_limit);
...
void select_charging_curret_aw3215dnr(void)
{
    if( g_temp_CC_value == Cust_CC_0MA)
    {
        pchr_turn_off_charging_aw3215dnr();
        xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery", "[BATTERY] charging current is set 0mA !\r\n");
    }
	
    mt_set_gpio_mode(GPIO_CHARGER_CURRENT_PIN,GPIO_MODE_GPIO);  
    mt_set_gpio_dir(GPIO_CHARGER_CURRENT_PIN,GPIO_DIR_OUT);
    mt_set_gpio_out(GPIO_CHARGER_CURRENT_PIN,GPIO_OUT_ZERO);
    
}

void ChargerHwInit_aw3215dnr(void)
{
    if (Enable_BATDRV_LOG == 1) {
    ...
    if(temp_init_flag==0)
    {
        if(g_enable_high_vbat_spec == 1)
        {
            if(g_pmic_cid == 0x1020)
                aw3215dnr_config_interface_liao(0x06,0x70);    
            else
                aw3215dnr_config_interface_liao(0x06,0x77);
        }
        else
            aw3215dnr_config_interface_liao(0x06,0x70);
        
        aw3215dnr_config_interface_liao(0x00,0x80);
        aw3215dnr_config_interface_liao(0x01,0xb9);
        ...
        if(g_enable_high_vbat_spec == 1)
        {
            if(g_pmic_cid == 0x1020)
                ...
                aw3215dnr_config_interface_liao(0x02,0xaa);
        }
        else
            aw3215dnr_config_interface_liao(0x02,0x8e);
        ...
        if(g_low_power_ready == 1)
            aw3215dnr_config_interface_liao(0x04,0x19);
        else
            aw3215dnr_config_interface_liao(0x04,0x1B); //194mA
...
#else
select_charging_curret_aw3215dnr();
...
void pchr_turn_on_charging_aw3215dnr(void)
{
       mt_set_gpio_mode(GPIO_CHARGER_EN_PIN,GPIO_MODE_GPIO);  
    mt_set_gpio_dir(GPIO_CHARGER_EN_PIN,GPIO_DIR_OUT);
    mt_set_gpio_out(GPIO_CHARGER_EN_PIN,GPIO_OUT_ZERO);



    if ( BMT_status.bat_charging_state == CHR_ERROR ) 
    {
        printk("[BATTERY:aw3215dnr] Charger Error, turn OFF charging !\r\n");
        pchr_turn_off_charging_aw3215dnr();
    }
    else if( (get_boot_mode()==META_BOOT) || (get_boot_mode()==ADVMETA_BOOT) )
    {   
        printk("[BATTERY:aw3215dnr] In meta or advanced meta mode, disable charging.\r\n");    
        pchr_turn_off_charging_aw3215dnr();
    }
    else
    {
        ChargerHwInit_aw3215dnr();
    
        if (Enable_BATDRV_LOG == 1) {
            printk("[BATTERY:aw3215dnr] pchr_turn_on_charging !\r\n");
        }
        ...
        if (g_bcct_flag == 1)
        {
            ...
            if (Enable_BATDRV_LOG == 1) {
                printk("[BATTERY:aw3215dnr] select_charging_curret_bcct !\n");
            }
        }
        else
        {
            select_charging_curret_aw3215dnr();
            if (Enable_BATDRV_LOG == 1) {
                printk("[BATTERY:aw3215dnr] select_charging_curret_aw3215dnr !\n");
            }
        }
        ...
        {
            if (Enable_BATDRV_LOG == 1) {
                printk("[BATTERY:aw3215dnr] charger enable !\r\n");
            }
        }
        else
        {
            //pchr_turn_off_charging_aw3215dnr();
            printk("[BATTERY:aw3215dnr] wait gFG_booting_counter_I_FLAG==2 (%d)\r\n", gFG_booting_counter_I_FLAG);
        }    
    }
}
...
void pchr_turn_off_charging_aw3215dnr (void)
{
    ...
    if(mt_usb_is_device())
    {
       ...
       mt_set_gpio_mode(GPIO_CHARGER_EN_PIN,GPIO_MODE_GPIO);  
    mt_set_gpio_dir(GPIO_CHARGER_EN_PIN,GPIO_DIR_OUT);
    mt_set_gpio_out(GPIO_CHARGER_EN_PIN,GPIO_OUT_ONE);
#else
...
#endif		
        if (Enable_BATDRV_LOG >= 1) {
            printk("[BATTERY] pchr_turn_off_charging_aw3215dnr !\r\n");
        }
...
int BAT_CheckBatteryStatus_aw3215dnr(void)
{
    ...
    if (Enable_BATDRV_LOG == 1) {
        printk("[BATTERY:ADC:aw3215dnr] VCHR:%d BAT_SENSE:%d I_SENSE:%d TBAT:%d (%d)\n", 
    ...
    if (Enable_BATDRV_LOG >= 1) {
        printk("[BATTERY:AVG:aw3215dnr] BatTemp:%d Vbat:%d VBatSen:%d SOC:%d ChrDet:%d Vchrin:%d Icharging:%d ChrType:%d USBstate:%d\r\n", 
    ...
    if (Enable_BATDRV_LOG == 1) {        
        printk("[BATTERY:FG:aw3215dnr] %d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d\r\n", 
           BMT_status.temperature ,BMT_status.bat_vol, BMT_status.ADC_BAT_SENSE, BMT_status.SOC, 
...
void BAT_thread_aw3215dnr(void)
{    
    kal_uint32 aw3215dnr_status=0;
    int i=0;
    int BAT_status = 0;
    //kal_uint32 tmp32;	
    int ret_val=0;
    ...
    if( upmu_is_chr_det() == KAL_TRUE )
    {
        ...
        if(BMT_status.charger_type == CHARGER_UNKNOWN)
        {
            ...
            if((CHR_Type_num==NONSTANDARD_CHARGER)&& (second_check==0)) 
	    {
		++second_check;
	        g_first_check=0;
		CHR_Type_num = mt_charger_type_detection(); 
    ...
    BAT_status = BAT_CheckBatteryStatus_aw3215dnr();
    ...
    {                    
        if(g_temp_status == TEMP_NEG_10_TO_POS_0)
        {
            aw3215dnr_config_interface_liao(0x02,0x52);   //for 3.9v CV threshold
        }
        else if(g_temp_status == TEMP_POS_10_TO_POS_45)
        {
            if(g_enable_high_vbat_spec == 1)
            {
                ...
                aw3215dnr_config_interface_liao(0x02,0x8E);  //for 4.2v CV threshold
        }
        else if((g_temp_status == TEMP_POS_0_TO_POS_10)||(g_temp_status == TEMP_POS_45_TO_POS_60))
        {     
            aw3215dnr_config_interface_liao(0x02,0x7A);  //for 4.1v CV threshold  mtk71259 20120720 FA
        }
        else{
            if(g_enable_high_vbat_spec == 1)
            {
    ...
    else
    {
        ...
        mt_set_gpio_mode(GPIO_CHARGER_STAT_PIN,GPIO_MODE_GPIO);  
        mt_set_gpio_dir(GPIO_CHARGER_STAT_PIN,GPIO_DIR_IN);
        aw3215dnr_status = mt_get_gpio_in(GPIO_CHARGER_STAT_PIN);
        
        /* check battery full */
        if( aw3215dnr_status != 0x0 )
        {
            if(BMT_status.bat_vol >= 3900)
            {
                ...
                g_HW_Charging_Done = 1;            
                //pchr_turn_off_charging_aw3215dnr();
                printk("[BATTERY:aw3215dnr] Battery real full and disable charging (%d) \n", aw3215dnr_status); 
                return;
            }
            else
            {
                printk("[BATTERY:aw3215dnr] HW Battery real full ?? (%d,%d) \n", aw3215dnr_status, BMT_status.bat_vol);
                pchr_turn_off_charging_aw3215dnr();
            }
        }
        ...
        pchr_turn_on_charging_aw3215dnr();        
        if (Enable_BATDRV_LOG >= 1) {
            printk("[BATTERY:aw3215dnr] Total charging timer=%ld \n", 
    ...
    if (Enable_BATDRV_LOG >= 1) {
        //aw3215dnr_dump_register();
    }

    if (Enable_BATDRV_LOG == 1) {
        printk("[BAT_thread_aw3215dnr] End ....\n");
    }
}
...
///////////////////////////////////////////////////////////////////////////////////////////
int g_FG_init = 0;
//extern unsigned int g_aw3215dnr_rdy_flag;
...
#if defined(CONFIG_POWER_EXT)			
			//if (g_aw3215dnr_rdy_flag) 
			{
				BAT_thread_aw3215dnr();
			}
#else
            if(g_FG_init == 0)
            {
