...
//atlasNew setting, 1.2A
#define  bq24158_AC_CHARGING_CURRENT_1050//56mo = 1.2A

int Enable_BATDRV_LOG = 1;

#if defined (ACER_C11)||defined(PHILIPS_W6500) //for 68 mo, if g_low_power_ready = 1, Iterm = 97mA else Iterm = 194mA  
int g_low_power_ready = 1;
#else
int g_low_power_ready = 0;
#endif

#if defined (ACER_C11)||defined(PHILIPS_W6500)
...
void ChargerHwInit_bq24158(void)
{
    ...
    if(temp_init_flag==0)
    {
        if(g_enable_high_vbat_spec == 1)
        {
			...
			#ifdef DEBUG_BQ24159
			                bq24158_config_interface_reg(0x06,0xf0);  
			#else
							xxx
...
#ifdef DEBUG_BQ24159       
        bq24158_config_interface_reg(0x05,0x02);
#else
        bq24158_config_interface_reg(0x05,0x04);		
#endif
...
void bq24158_set_ac_current(void)
{
    ...
    if (Enable_BATDRV_LOG == 1) {
        printk("[BATTERY:bq24159] bq24158_set_ac_charging_current  -1.1\r\n");    
    }    

    bq24158_config_interface_reg(0x06,0x60); //set ISAFE, 0x60 is 1.4A for 56R
    bq24158_config_interface_reg(0x01,0xF8);
    bq24158_config_interface_reg(0x02,0xBE);//8e -> be
	//bq24158_config_interface_reg(0x02,0xaa);  for hv battery test
    bq24158_config_interface_reg(0x04,0x2a); //set IOCHARGE 0x2a is 1.27A for 56R
    bq24158_config_interface_reg(0x05,0x02);
...
{
		printk("kernel, select_charging_curret_bq24158  -0  charger_type=%d \r\n",BMT_status.charger_type);    
...
//punk add
			...
			if(g_boot_mode == KERNEL_POWER_OFF_CHARGING_BOOT || g_boot_mode == LOW_POWER_OFF_CHARGING_BOOT)
			{
				...
				printk("select_charging_curret_bq24158 -11 charger_vol=%d,charger_compatible_500mA_counter=%d \r\n",BMT_status.charger_vol,charger_compatible_500mA_counter);    

				if (BMT_status.charger_vol <= V_CHARGER_COMPATIBLE_V ){
					bq24158_config_interface_reg(0x01,0x78);//input current limit 500mA
				printk("select_charging_curret_bq24158 -11.11 charger_vol=%d,charger_compatible_500mA_counter=%d \r\n",BMT_status.charger_vol,charger_compatible_500mA_counter);    
				}
				else{
				printk("select_charging_curret_bq24158 -11.2 charger_vol=%d,charger_compatible_500mA_counter=%d \r\n",BMT_status.charger_vol,charger_compatible_500mA_counter);    
				}
				...
				printk("select_charging_curret_bq24158 -12 charger_vol=%d,charger_compatible_500mA_counter=%d \r\n",BMT_status.charger_vol,charger_compatible_500mA_counter);    
			}
			else
#endif
			{
				printk("select_charging_curret_bq24158  charger_type=%d,g_is_500mA_charger=%d \r\n",BMT_status.charger_type,g_is_500mA_charger);    
			   if ((BMT_status.charger_type == STANDARD_CHARGER) || (BMT_status.charger_type == CHARGING_HOST))
			   {
					printk("select_charging_curret_bq24158 -2 charger_vol=%d \r\n",BMT_status.charger_vol); 
				 if (g_is_500mA_charger == 1)
				 {
					printk("select_charging_curret_bq24158 -3 charger_vol=%d \r\n",BMT_status.charger_vol);  
						bq24158_config_interface_reg(0x01,0x78);//input current limit 500mA
				 } 
			   }
